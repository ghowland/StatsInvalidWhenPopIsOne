...
Every time someone applies a statistical value to a single case, they are ignoring all the local data available to them, and choose to use aggregated external data instead of the local data to make a decision.  This aggregated external data is assured to be only coincidentally related to the local data, as it is not derived from the local data.
